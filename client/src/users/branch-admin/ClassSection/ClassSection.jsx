...
import { fetchSubjects, deleteSubject } from '../../../api/subjectApi';
...
const ClassSection = () => {
    ...
    const filteredSubjectData = subjects.filter((item) => {
        return (
            item.subjectName.toLowerCase().includes(subjectFilterText.toLowerCase()) ||
            item.sectionId?.sectionName.toLowerCase().includes(subjectFilterText.toLowerCase())
        );
    });
    ...
    const subjectColumns = [
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="flex space-x-3">
                    <AiFillEye className="text-blue-500 cursor-pointer" onClick={() => handleSubjectViewClick(row._id)} />
                    <AiFillEdit className="text-green-500 cursor-pointer" onClick={() => handleEditSubjectClick(row._id)} />
                    <AiFillDelete className="text-red-500 cursor-pointer" onClick={() => handleSubjectDelete(row._id)} />
                </div>
            ),
            ignoreRowClick: true,
            allowOverflow: true,
