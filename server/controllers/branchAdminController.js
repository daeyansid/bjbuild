...
exports.updateBranchAdmin = async (req, res) => {
    upload.single('photo')(req, res, async (err) => {
        ...
        const { id } = req.params;
        const { fullName, cnicNumber, phoneNumber, address, salary, gender, password, joinDate, adminId } = req.body;
        const photo = req.file ? req.file.filename : null; // Save only the filename
        ...
        try {
            ...
            branchAdmin.adminId = adminId || branchAdmin.adminId;
            if (photo) branchAdmin.photo = photo; // Update photo if a new file is uploaded
